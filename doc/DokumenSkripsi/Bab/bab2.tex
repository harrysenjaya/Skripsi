...
\begin{itemize}
	    \item \texttt{Map<String,String> cookies()} \\
		Berfungsi untuk mendapatkan seluruh \textit{cookies}. \\
...
\subsection{FXML}
\label{fxml}
FXML adalah bahasa \textit{markup} berbasis XML yang dapat dituliskan untuk membangun grafik objek Java. FXML memberikan alternatif yang nyaman untuk membuat grafik dalam kode prosedural, dan cocok untuk mendefinisikan antarmuka pengguna aplikasi JavaFX, karena struktur hierarki dari dokumen XML sangat mirip dengan struktur grafik \textit{scene} JavaFX. \cite{javafx} Subbab-subbab berikut menjelaskan beberapa bagian dari FXML. \cite{fxml}

\subsubsection{Controller Method Event Handlers}
\textit{Controller Method Event Handlers} adalah \textit{method} yang ditentukan oleh "pengontrol" dokumen. \textit{Controller} adalah objek yang dikaitkan dengan konten \textit{deserialized} dari dokumen FXML dan bertanggung jawab untuk mengoordinasikan perilaku objek (seringkali elemen antarmuka pengguna) yang ditentukan oleh dokumen. Sebuah \textit{event handler metode controller} ditentukan oleh simbol \textit{hash} terkemuka diikuti dengan nama \textit{method}.

\subsubsection{Variable Resolution}
Dokumen FXML mendefinisikan \textit{namespace} variabel dimana elemen bernama dan variabel  dapat diidentifikasi secara unik. Menetapkan nilai fx:id ke elemen akan membuat variabel dokumen yang nantinya dapat dirujuk oleh atribut dereferensi variabel. Selain itu, jika tipe objek mendefinisikan properti \texttt{id}, nilai ini juga akan diteruskan ke \textit{method} \texttt{setId()} milik objek.

\section{SIAModels}
\label{sec:siamodels}

SIAModels merupakan kelas-kelas dalam bahasa Java yang merepresentasikan objek-objek yang tersedia di Sistem Informasi Akademik UNPAR \cite{siamodels}. Pada skripsi ini akan digunakan SIAModels versi 5.1.1.
